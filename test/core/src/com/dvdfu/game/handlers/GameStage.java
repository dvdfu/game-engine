...
public class GameStage extends Stage {
	...
	private void updateCam() {
		float camX = getCamera().position.x
				+ (camPosition.x - getCamera().position.x) * camSmoothing;
		float camY = getCamera().position.y
				+ (camPosition.y - getCamera().position.y) * camSmoothing;
		if (camShake > 0) {
	...
	public void draw() {
		...
		// getSpriteBatch().draw(Sprites.plain.getFrame(0), getCamX() -
		// Consts.ScreenWidth / 2,
		// getCamY() - Consts.ScreenHeight / 2, Consts.ScreenWidth,
		// Consts.ScreenHeight);
		getSpriteBatch().end();
		super.draw();
	}
